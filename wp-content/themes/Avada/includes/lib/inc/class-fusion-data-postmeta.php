...
class Fusion_Data_PostMeta {
	...
	public function get( $option, $args = [] ) {
		...
		if ( isset( $meta[ $option ] ) ) {
			return apply_filters( 'get_post_metadata', $meta[ $option ], $this->post_id, $option, true );
		}
		...
		if ( false !== strpos( $option, '[' ) ) {
			...
			if ( ! isset( $meta[ $parts[0] ] ) ) {
				return apply_filters( 'get_post_metadata', '', $this->post_id, $option, true );
			}
			$value = $meta[ $parts[0] ];
			unset( $parts[0] );
			...
			return apply_filters( 'get_post_metadata', $value, $this->post_id, $option, true );
		}
